...
from edward_tools.coupled_fq_potential import coupled_flux_qubit_non_linear_approx_pot, coupled_flux_qubit_non_linear_approx_force, coupled_flux_qubit_non_linear_approx_pot_break_down, coupled_flux_qubit_non_linear_approx_force_break_down
from edward_tools.coupled_fq_potential import coupled_flux_qubit_non_linear_approx_pot_only_d_beta
...
def find_minimum_of_four_well(cfqr, _t, beta_1, beta_2, d_beta_1, d_beta_2, verbose = True):
    ...
    if verbose:
        print(f"time: {_t}, _phi_1dcx: {_phi_1dcx}, _phi_2dcx: {_phi_2dcx}, _M_12: {_M_12}")
...
def find_minimum_of_conditional_tilt_in_Y(cfqr, _t, beta_1, beta_2, d_beta_1, d_beta_2, verbose = True):
    ...
    def Fcn_conditional_tilt_in_y(x):
        return [_xi * x[0] - beta_1 * np.sin(x[0]) + _M_12 * _xi * x[1],
...
def find_minimum_of_raise_barrier(cfqr, _t, beta_1, beta_2, d_beta_1, d_beta_2, verbose = True):
    ...
    _M_12 = _params_at_t[12]
    _xi = 1 / (1 - _M_12**2)
    print(_phi_1dcx, _phi_2dcx, _M_12)

    def Fcn_raise_barrier(x):
        return [_xi * x[0] - beta_1 * np.sin(x[0]) + _M_12 * _xi * x[1],
                _xi * x[1] - beta_2 * np.sin(x[1]) + _M_12 * _xi * x[0]
               ]
    ...
    def Fcn_conditional_tilt_in_x(x):
        return [
              _xi * x[0] - beta_1 * np.sin(x[0]) * np.cos(_phi_1dcx/2) + d_beta_1 * np.cos(x[0]) * np.sin(_phi_1dcx/2) 
                    + _M_12 * _xi * x[1],
              _xi * x[1] - beta_1 * np.sin(x[1]) + _M_12 * _xi* x[0],
        ]
