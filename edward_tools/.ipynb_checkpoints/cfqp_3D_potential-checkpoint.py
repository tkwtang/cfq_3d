...
def coupled_flux_qubit_3D_non_linear_approx_pot(phi_1, phi_2, phi_3, phi_1dc, phi_2dc, phi_3dc, params):
    """
    8D 4-well potential.
    ...
    u_21 = 1/2 * xi_123 * (phi_2 - phi_2x)**2
    u_22 = 1/2 * gamma_2 * (phi_2dc - phi_2xdc)**2
    u_23 = beta_2 * np.cos(phi_2) * np.cos(phi_2dc/2)
    u_24 = d_beta_2 * np.sin(phi_2) * np.sin(phi_2dc/2)

    u_31 = 1/2 * xi_123 * (phi_3 - phi_3x)**2
    u_32 = 1/2 * gamma_3 * (phi_3dc - phi_3xdc)**2
    u_33 = beta_3 * np.cos(phi_3) * np.cos(phi_3dc/2)
    u_34 = d_beta_3 * np.sin(phi_3) * np.sin(phi_3dc/2)

    u_4 = xi_123 * ((mu_12 + mu_13 * mu_23) * (phi_1 - phi_1x) * (phi_2 - phi_2x) + (mu_13 + mu_12 * mu_23) * (phi_1 - phi_1x) * (phi_3 - phi_3x) + (mu_23 + mu_12 * mu_13) * (phi_2 - phi_2x) * (phi_3 - phi_3x))
...
def coupled_flux_qubit_3D_non_linear_approx_force(phi_1, phi_2, phi_3, phi_1dc, phi_2dc, phi_3dc, params):
    """
    8D 4-well potential.
