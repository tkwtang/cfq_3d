...
def simulateSingleCoupledFluxQubit(params, initial_parameter_dict, protocol_list, potential = None, potential_default_param = None,
    ...
    phi_1_dcx = 0, phi_2_dcx = 0, initial_state = None, manual_domain = None, comment = "", measure_all_states = False, has_velocity = True, modifiedFunction=None, fidelityTest = True, pickle_save_path = None):
    ...
    export_params = {
        ...
        'as_step': params["as_step"], 'percentage': params["percentage"], "mass": params["mass"]
    }
...
def saveSimulationResult(simResult, U0_1, timeOrStep = "time", save = False, save_final_state = False, comment = "", saveFolderPath = "coupled_flux_qubit_protocol/coupled_flux_qubit_data_gallery", save_all_state = False, save_work_distribution_time_array = False, save_animation = False):
    """U0_1"""
    #  fidelity
    ...
    params_save_field = ["N", "dt", "comment", "protocol_time_array", "protocol_time_index_array"]
    ...
    with open(f"{saveFolderPath}/{simResult['simulation_data']['simulation_id']}_waiting_to_be_integrated_to_gallery.json", "w+") as fw:
