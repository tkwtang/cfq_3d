...
class SimManager:
    ...
    def run_sim(self, verbose=True, init_state = None,  manual_domain = None, axes = None, percentage = 0.1, as_step = np.s_[::],  **kwargs):
        # percentage is a keyword for the percentage of sample data will be used
        # as_step is added here
        ...
        if verbose:
            print('\n initializing...')
        self.initialize_sim()
        self.set_sim_attributes(init_state = init_state, manual_domain = manual_domain, axes = axes, percentage = percentage, as_step = as_step)
